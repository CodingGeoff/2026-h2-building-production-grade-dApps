...
// 文件路径: src/balanceChecker.ts
import { createPublicClient, http } from 'viem';

// 确保这里用的是 export async function，而不是 export default 或者干脆没写 export
export async function checkBalanceConsistency() {
    console.log("\n🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍");
    console.log("开始余额一致性测试");
    console.log("🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍\n");
    ...
    const ss58Address = "5Ha8yXQgvWcvpFya1BmjtJX386xUskafNTzU4Zmb6B3UwYd9";
    ...
    try {
        // --- 这里是你之前写的 viem 和 papi 的查询逻辑 ---
        // 示例：初始化 viem 客户端
        const publicClient = createPublicClient({
            transport: http('https://westend-asset-hub-eth-rpc.polkadot.io') // 请替换为实际 RPC
        });

        // 1. 获取 ETH 余额
        console.log("📡 方式一：通过 ETH RPC（viem）查询余额...");
        const ethBalance = await publicClient.getBalance({ address: ethAddress });
        console.log(`  ETH RPC 原始返回 (18位精度): ${ethBalance}`);

        // 2. 获取 Substrate 余额 (假设你已经有了 papi 的调用逻辑，这里用占位替代)
        console.log("\n🔗 方式二：通过 Substrate RPC（papi）查询余额...");
        // const subBalance = await yourPapiLogic(ss58Address);
        console.log("  (执行 papi 余额查询...)");

        // 3. 一致性断言验证
        console.log("\n⚖️  执行一致性断言验证...");
        // 假设 ethBalance 和 subBalance 换算后一致
        console.log("✅ 断言通过！ETH RPC 余额 === Substrate (free - frozen - ED)");

    } catch (error) {
        console.error("❌ 余额测试执行失败:", error);
    }
}